...
- declare the LaunchControl XL MIDI device match in `main.mlperformance`
- use script-authored MIDI CC bindings to target the loaded performance control
- map fader values as absolute normalized values, not repeated step nudges
- clamp and snap fader values to the control's authored range and step
- map button presses to typed increment/decrement steps
...
- preserve enough typed action information for later replay without the
  controller attached
